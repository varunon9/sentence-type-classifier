...
function isImperative(taggedWords) {
	...
	var firstTag = getTag(taggedWords, 0);
	if (firstTag == "VB" || firstTag == "VBP") {
		imperative = true;
	} else if (firstTag == "RB") {
		...
		if (secondTag) {
			if (secondTag == ",") {
				...
				// check for third tag
				var thirdTag = getTag(taggedWords, 2);
				if (thirdTag) {
					if (thirdTag == "VB" || thirdTag == "VBP") {
						imperative = true;
					} else if (thirdTag == "RB") {

						// check for 4th tag
						var fourthTag = getTag(taggedWords, 3);
						if (fourthTag) {
							if (fourthTag == "VB" || fourthTag == "VBP") {
								imperative = true;
							}
						}
					}
